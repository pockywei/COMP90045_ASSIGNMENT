...
let check_expr_type hash_table expr = match expr with
  ...
  | Ebinop(expr,binop,expr) ->(check_expr_type hash_table expr;check_expr_type hash_table expr)
  | Eunop(unop,expr) -> check_expr_type hash_table expr
  ...
  | Ebracket(expr) ->check_expr_type hash_table hash_table expr
...
let get_lvalue_symbol_type hash_table lvalue = match lvalue with
...
let check_ref_val_type_equal first second = match first with 
  | S_Ref_Hash (bean_type,inner_hash_table) -> match second with
  	|S_Hash (bean_type_1,inner_hash_table_1) -> if inner_hash_table = inner_hash_table_1
  		then true
  		else false 
  	|S_Ref_Hash (bean_type_1,inner_hash_table_1) -> if inner_hash_table = inner_hash_table_1
  		then true
  		else false 
  	| _ -> false
...
let rec convert_one_expr_param_to_symbol_type one_expr = match one_expr with 
  ...
  | Eint(_) -> S_Int(Int,-1) 
  | Elval(lvalue) -> get_lvalue_stack_num lvalue
  | Ebinop (expr_1,binop,expr_2) -> (convert_one_expr_param_to_symbol_type expr_1; convert_one_expr_param_to_symbol_type expr_2)
...
let gen_symbol_type_to_register symbol_type is_ref_caller is_ref_callee   =
	match (is_ref_caller,is_ref_callee) with
	| (true,false) -> match symbol_type with 		
		| S_Bool(_ , stack_num) -> (incr register_count;
			print_load (get_register_string cur_register) stack_num; (* r = s , s is addr *)
			print_load_indirect (get_register_string cur_register) (get_register_string cur_register) (* dereference r *))
		| S_Int(_ , stack_num) -> (incr register_count;
			print_load (get_register_string cur_register) stack_num; (* r = s , s is addr *)
			print_load_indirect (get_register_string cur_register) (get_register_string cur_register) (* dereference r *))
		...
		| S_Intext_Hash(inner_hash_table) -> faHashtbl.iter (fun key value -> gen_symbol_type_to_register value  is_ref_caller is_ref_callee ) inner_hash_table lse
		...
		| S_Ref_Int(_ , stack_num) -> (incr register_count;
			print_load (get_register_string cur_register) stack_num; (* r = s , s is addr *)
			print_load_indirect (get_register_string cur_register) (get_register_string cur_register) (* dereference r *))
		| S_Ref_Bool(_ , stack_num) -> (incr register_count;
			print_load (get_register_string cur_register) stack_num; (* r = s , s is addr *)
			print_load_indirect (get_register_string cur_register) (get_register_string cur_register) (* dereference r *)) (*only return Bool or Int, typedef of {} type will cause error*)
	...
	| (false,true) -> match symbol_type with 		(* caller not ref, callee ref*)
		| S_Bool(_ , stack_num) -> (incr register_count;
			print_load_address (get_register_string cur_register) stack_num; (* r = &s *))
		| S_Int(_ , stack_num) -> (incr register_count;
			print_load_address (get_register_string cur_register) stack_num; (* r = &s *))
		| S_Hash(_,inner_hash_table) -> Hashtbl.iter (fun key value -> gen_symbol_type_to_register value  is_ref_caller is_ref_callee ) inner_hash_table 
		...
		| S_Ref_Int(_ , stack_num) -> (incr register_count;
			print_load_address (get_register_string cur_register) stack_num; (* r = &s *))
		| S_Ref_Bool(_ , stack_num) -> (incr register_count;
			print_load_address (get_register_string cur_register) stack_num; (* r = &s *)) (*only return Bool or Int, typedef of {} type will cause error*)
		| S_Ref_Intext_Hash(inner_hash_table) -> Hashtbl.iter (fun key value -> gen_symbol_type_to_register value  is_ref_caller is_ref_callee ) inner_hash_table 
		| _ -> (Printf.printf "error on gen_symbol_type_to_register\n";exit 0)
	| (true,true) -> match symbol_type with (* caller ref, callee ref *)
		| S_Bool(_ , stack_num) -> (incr register_count;
			print_load (get_register_string cur_register) stack_num; (* r = s *))
		| S_Int(_ , stack_num) -> (incr register_count;
			print_load (get_register_string cur_register) stack_num; (* r = s *))
		| S_Hash(_,inner_hash_table) -> Hashtbl.iter (fun key value -> gen_symbol_type_to_register value  is_ref_caller is_ref_callee ) inner_hash_table 
		| S_Intext_Hash(inner_hash_table) -> Hashtbl.iter (fun key value -> gen_symbol_type_to_register value  is_ref_caller is_ref_callee ) inner_hash_table 
		...
		| S_Ref_Int(_ , stack_num) -> (incr register_count;
			print_load (get_register_string cur_register) stack_num; (* r = s *))
		| S_Ref_Bool(_ , stack_num) -> (incr register_count;
			print_load (get_register_string cur_register) stack_num; (* r = s *)) (*only return Bool or Int, typedef of {} type will cause error*)
		| S_Ref_Intext_Hash(inner_hash_table) -> Hashtbl.iter (fun key value -> gen_symbol_type_to_register value  is_ref_caller is_ref_callee ) inner_hash_table 
		| _ -> (Printf.printf "error on gen_symbol_type_to_register\n";exit 0)
...
let process_calling_method_param caller_hash_table callee_hash_table one_expr_param one_param = match one_param with
	| (Val,_,one_callee_param_name) -> (cur_expr_type := BeanTypeNone;
		check_expr_type one_expr_param;(*make sure has same type*)
		if check_ref_val_type_equal (Hashtbl.find callee_hash_table one_callee_param_name) (convert_one_expr_param_to_symbol_type one_expr_param)
		then match one_expr_param with
		  ...
		  | Elval(lvalue) -> if cur_expr_type = Int || cur_expr_type = Bool 
		  	...
		  	else  (Hashtbl.iter (fun key value -> gen_symbol_type_to_register (get_lvalue_ref_or_not lvalue) false ) get_hash_table_symbol (get_lvalue_symbol_type lvalue))
	...
	| (Ref,_,one_callee_param_name) ->  (cur_expr_type := BeanTypeNone;
		...
		(* ref does not allow primitive value pass in 
		  ...
		  | Elval(lvalue) -> if cur_expr_type = Int || cur_expr_type = Bool 
		  	then  codegen_arithmatic one_expr_param
		  	else  (Hashtbl.iter (fun key value -> gen_symbol_type_to_register (get_lvalue_ref_or_not lvalue) true ) get_hash_table_symbol (get_lvalue_symbol_type lvalue)) (* passed in is a struct / typedef *)
...
let rec codegen_one_stmt hash_table one_stmt = match one_stmt with
	| Assign(lvalue, rvalue) -> (process_rvalue (get_lvalue_ref_or_not lvalue hash_table lvalue) lvalue rvalue) (*set cur_expr_type to lvalue type, then in process_rvalue will check type*)
  ...
  | Method(method_name, expr_params_list) ->(register_count := -1;
  	...
  	let temp_hash_symbol_table = get_hash_table_symbol (Hashtbl.find symbol_table_hash method_name) in (*callee hashtable*)
  		let temp_param_list = Hashtbl.find func_param_order_hash_table method_name in(*(valRef*typedefStruct*string) list*) 
		  	if List.length paramList = List.length temp_param_list
		  	then List.iter2 (fun first second -> (process_calling_method_param hash_table temp_hash_symbol_table first second)) expr_params_list temp_param_list )
...
let rec find_funcdef funcdefs func_name = try match (List.hd funcdefs) with
		| ((fun_name,_),_,_) -> if fun_name = func_name then List.hd funcdefs  else find_main_funcdef (List.tl funcdefs) 
...
let start_test_analyzer prog = (build_typedef_table_hash (prog.typedefs);
		...
		start_translate_by_function (find_main_funcdef (prog.funcdefs) "main");
